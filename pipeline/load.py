...
class Load(luigi.Task):
    ...
    def run(self):
        ...
        try:
            # Read query to truncate pactravel_source schema in dwh
            truncate_query = read_sql_file(
                file_path = f'{DIR_LOAD_QUERY}/pactravel_source-truncate_tables.sql'
            )

            # Read load query to pactravel_staging schema

            aircafts_query = read_sql_file(
                file_path = f'{DIR_LOAD_QUERY}/stg_aircrafts.sql'
            )
            
            airlines_query = read_sql_file(
                file_path = f'{DIR_LOAD_QUERY}/stg_airlines.sql'
            )
            
            airports_query = read_sql_file(
                file_path = f'{DIR_LOAD_QUERY}/stg_airports.sql'
            )
            
            customers_query = read_sql_file(
                file_path = f'{DIR_LOAD_QUERY}/stg_customers.sql'
            )
            
            hotel_query = read_sql_file(
                file_path = f'{DIR_LOAD_QUERY}/stg_hotel.sql'
            )
            
            flight_bookings_query = read_sql_file(
                file_path = f'{DIR_LOAD_QUERY}/stg_flight_bookings.sql'
            )
            
            hotel_bookings_query = read_sql_file(
                file_path = f'{DIR_LOAD_QUERY}/stg_hotel_bookings.sql'
            )
        ...
        try:            
            ...
            logging.info(f"Truncate pactravel_source Schema in DWH - SUCCESS")
        
        except Exception:
            logging.error(f"Truncate pactravel_source Schema in DWH - FAILED")
            
            raise Exception("Failed to Truncate_source pactravel Schema in DWH")
        ...
        try:
            ...
            try:
                # Load to pactravel_source schema in DWH
